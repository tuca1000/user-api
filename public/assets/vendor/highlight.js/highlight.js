...
const HLJS = function (hljs) {
  ...
  function _highlight(languageName, code, ignoreIllegals, continuation) {
    ...
    function doEndMatch(match) {
      ...
      const end_mode = endOfMode(top, match, matchPlusRemainder)
      if (!end_mode) {
        return NO_MATCH
      }
  ...
  */
  function highlightAuto(code, languageSubset) {
    ...
    let result = justTextHighlightResult(code)
    let secondBest = result
    languageSubset
      .filter(getLanguage)
      .filter(autoDetection)
      .forEach(function (name) {
        const current = _highlight(name, code, false)
  ...
  */
  function highlightBlock(block) {
    ...
    fire('before:highlightBlock', { block: block, language: language })
